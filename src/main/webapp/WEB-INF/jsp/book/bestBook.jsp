		...
		<div class="d-flex justify-content-center">
		  	<ul class="pagination">
			    <c:if test="${pageMaker.prev}">
			        <li>
			            <a href="/book/best_book_view${pageMaker.makeQuery(pageMaker.startPage - 1)}&cid=${cid}" class="mr-2 text-dark">[이전]</a>
			        </li>		         
			    </c:if>
			    ...
			    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="index">
			        ...
			        <a href="/book/best_book_view${pageMaker.makeQuery(index)}&cid=${cid}" class="text-danger">[${index}]</a>
			        </c:when>
			        <c:otherwise>
			        <a href="/book/best_book_view${pageMaker.makeQuery(index)}&cid=${cid}">[${index}]</a>
			        </c:otherwise>
			        </c:choose>
			    </c:forEach>
			    ...
			    <c:if test="${pageMaker.next}">
			        <li>
			            <a href="/book/best_book_view${pageMaker.makeQuery(pageMaker.endPage + 1)}&cid=${cid}" class="ml-2 text-dark">[다음]</a>
			        </li>
			    </c:if>   
			</ul>
		</div>
