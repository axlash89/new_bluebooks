...
<%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
		<h3 class="text-center pt-3 category-guide font-weight-bold">상세검색 결과입니다.</h3>
		<hr>
		
		<c:if test="${not empty advancedSearchedBookList}">
			...
			<div class="d-flex justify-content-center mt-3 mb-2">
			  	<ul class="pagination">
				    <c:if test="${pageMaker.prev}">
				        <li>
				            <a href="/book/advanced_searched_result_view${pageMaker.makeQuery(pageMaker.startPage - 1)}&title=${title}&author=${author}&publisher=${publisher}&pubPeriod=${pubPeriod}" class="a-tag-deco-prev-paging">[이전]</a>
				        </li>		         
				    </c:if>
				    ...
				    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="index">
				        ...
				        <a href="/book/advanced_searched_result_view${pageMaker.makeQuery(index)}&title=${title}&author=${author}&publisher=${publisher}&pubPeriod=${pubPeriod}" class="a-tag-deco-now-paging">[${index}]</a>
				        </c:when>
				        <c:otherwise>
				        <a href="/book/advanced_searched_result_view${pageMaker.makeQuery(index)}&title=${title}&author=${author}&publisher=${publisher}&pubPeriod=${pubPeriod}" class="a-tag-deco-paging">[${index}]</a>
				        </c:otherwise>
				        </c:choose>
				    </c:forEach>
				    ...
				    <c:if test="${pageMaker.next}">
				        <li>
				            <a href="/book/advanced_searched_result_view${pageMaker.makeQuery(pageMaker.endPage + 1)}&title=${title}&author=${author}&publisher=${publisher}&pubPeriod=${pubPeriod}" class="a-tag-deco-next-paging">[다음]</a>
				        </li>
				    </c:if>   
				</ul>
			</div>
